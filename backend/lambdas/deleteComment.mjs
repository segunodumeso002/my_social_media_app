...
export const handler = async (event) => {
  ...
  try {
    ...
    if (!postId || !commentId || !Number.isFinite(createdAt)) {
      return {
        statusCode: 400,
        headers: getCorsHeaders(event),
        body: JSON.stringify({ message: "postId, commentId, and createdAt are required" })
      };
    }
    ...
    return {
      statusCode: 200,
      headers: getCorsHeaders(event),
      body: JSON.stringify({ success: true, deletedCommentId: commentId })
    };
  } catch (error) {
